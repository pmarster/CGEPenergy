...
var dataSets = ["White", "Black", "Latino", "Asian", "Multiracial", "Bernie Event", "Obama 2012", "Obama 2008", "Turnout", "Name"];
...
function makeMyMap(error, districtListData, ushex, ddata, presidentialData, turnOutData) {
	...
	turnOutData.forEach(function (d) {
		...
		d.Total = +d.Total;
		d.Name = d.Name;
		dataByDistrictID[d.districtID].push(d.Total,d.Name);
	});
...
function showDataSet(i) {
		d3.select(".header").text("Percent Made Whole");
	if (i != 5)	{
		buildColorRange(i);
...
function changeTooltip(d) {
	if (d.properties.state != "Ocean") { // if you ARE on a district
		...
		document.getElementById("imageleg").src = "Images/" + d.properties.state + getRealDistrict(d.properties.district, d.properties.state) + ".jpg";
		for (i = 0; i < 10; i++) {
			var classNameSplit = dataSets[i].split(" ");
			if (i == 9) {
				d3.select("." + dataSets[i] + ".Tooltip").text("Rep: " + dataByDistrictID[d.properties.districtID][i]);
			} 
			else {
			if (i == 8) {
			d3.select("." + dataSets[i] + ".Tooltip").text("% Made Whole: " + dataByDistrictID[d.properties.districtID][i]);
			}
			else {
